...
func MostGPURequestedPriorityMap(pod *v1.Pod, meta interface{}, nodeInfo *schedulercache.NodeInfo) (schedulerapi.HostPriority, error) {
	nonZeroRequest := &schedulercache.Resource{}
	for i := range pod.Spec.Containers {
		container := &pod.Spec.Containers[i]
		gpu := container.Resources.Limits.NvidiaGPU().Value()
		nonZeroRequest.NvidiaGPU += gpu
	}
...
func calculateGPUUsedPriority(pod *v1.Pod, podRequests *schedulercache.Resource, nodeInfo *schedulercache.NodeInfo) (schedulerapi.HostPriority, error) {
	node := nodeInfo.Node()
	if node == nil {
	...
	totalResources.NvidiaGPU += nodeInfo.RequestedResource().NvidiaGPU
	gpuScore := calculateGPUUsedScore(totalResources.NvidiaGPU, allocatableResources.NvidiaGPU, node.Name)
	if glog.V(10) {
		...
		glog.V(10).Infof(
			...
			pod.Name, node.Name,
			allocatableResources.NvidiaGPU,
			totalResources.NvidiaGPU,
			gpuScore,
		)
	}
